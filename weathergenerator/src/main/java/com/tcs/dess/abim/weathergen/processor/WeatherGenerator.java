...
public class WeatherGenerator extends Thread 
{
    public static void main( String[] args ) throws FileNotFoundException
    {
    	...
    	// Reads the files containing location details and weather condition details and load to a bean
    	//Eg.Melbourne,-37.82,144.95,9 (City,Lat,Long,Elevation)
    	//Eg.Rain,+10,+20,1005,1010,90,100 (Weather,Temphigh,Templow,Pressurehigh,Pressurelow,humidityhigh,humiditylow)
        ...
        while (loop_const<1000)
        {
        	...
        	//randomly select the climate either sunny,rain,snow,cloudy,clear etc. 
        	...
        	//randomly generate values for temp,pressure,humidity based on high/low values specified. 
        	resultString.append(Formatter.check_sym(RandNum.rand_float(condlist.getConditionlist().get(x).getTemp_low(),condlist.getConditionlist().get(x).getTemp_high()))+"|");
        	resultString.append(Formatter.check_precision(RandNum.rand_float(condlist.getConditionlist().get(x).getPres_low(),condlist.getConditionlist().get(x).getPres_high()))+"|");
        	resultString.append(RandNum.rand_int(condlist.getConditionlist().get(x).getHumd_low(),condlist.getConditionlist().get(x).getHumd_high())+"\n");
    ...
    public void readFiles(String fileName1, String fileName2, Locationslist loclist, Conditionslist condlist) throws FileNotFoundException {
    	...
    	try {
    		...
    		//load the list of values to the location and the condition list bean
